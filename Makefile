...
server: deps
	go install -tags '$(BUILDTAGS)' -ldflags "-X ngrok/server.defaultServer=$(DOMAIN)" ngrok/main/ngrokd

fmt:
	go fmt ngrok/...

client: deps
	go install -tags '$(BUILDTAGS)' -ldflags "-X ngrok/client.defaultServerAddr=$(DOMAIN):4443" ngrok/main/ngrok
